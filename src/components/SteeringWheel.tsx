...
export const SteeringWheel: React.FC<SteeringWheelProps> = ({ props }) => {
    ...
    var degree: number = 0;
    if(props.options.realtime){         // If the real time option is set it gets the last value 
    ...
    return (
        <div>
            <svg height={height} width={width}
                viewBox="0 0 374.43 212.87" transform={"rotate(" + degree + ")"}
                ...
                <text transform='translate(200 111.8)' fill='#fff' fontSize={87} textAnchor='middle'>{Math.round(degree)}°</text> 
            </svg>
        </div>);
}
