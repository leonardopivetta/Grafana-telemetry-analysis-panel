...
export const SteeringWheel: React.FC<SteeringWheelProps> = ({ props }) => {
    ...
    props.eventBus.getStream(DataHoverEvent).subscribe((data) => { setTime(data.payload.point.time) })  // Bus for reading the poistion of the cursor
    props.onOptionsChange.bind((e: any)=> {setTime(0)});
    ...
    const deegree = view.get(nearestIndex)[1];      // [0] is the time, [1] is the value
     ...
     const color = "hsl(" + (120 - Math.abs(deegree)) + ", 100%, 50%)"; // Color using the hsl encoding

    return (
        <div>
            <svg height={height} width={width}
                viewBox="0 0 374.43 212.87" transform={"rotate(" + deegree + ")"}
                style={isHorizontal ? { paddingLeft: padding, paddingRight: padding } : { paddingTop: padding, paddingBottom: padding, }}>
                <path fill={color}      // Outer path (coloured)
                ...
                <path fill="#000"       // Inner path (black)
                ...
                <text transform='translate(200 111.8)' fill='#fff' fontSize={87} textAnchor='middle'>{Math.round(deegree)}°</text> 
            </svg>
        </div>);
}
